...
public final class DOCXHelper
{
        ...
        if (n != null)
        {
            ...
            if (parent != null)
            {
                ...
                while (nTmp != sibling)
                {
                    ...
                    if (nTmp != null && WORD_STYLE_TEXT_W.equals(nTmp.getNodeName()))
                    {
    ...
    /**
     * Gets the text node for style node. The process will avoid forbidden properties (rPr) if it matches it the process
     * will conitnue with the sibling of the parent of the current tag
     * 
     * @param n the style node
     * 
     * @return the text node for style node
    ...
    public static String getTextNodeForStyleNode(Node n, String forbiddenProperties)
    {
        ...
        if (n != null && n.getParentNode() != null)
        {
            ...
            if (parent != null)
            {
                ...
                {
                    nTmp = XMLHelper.next(nTmp);
                    if (forbiddenProperties != null && forbiddenProperties.length() > 0 && WORD_STYLE_PARAGRAPH_RPR.equals(nTmp.getNodeName()))
                    {
    ...
    public static Node getNodeForStyleNode(Node n)
    {
        ...
        if (WORD_STYLE.equals(XMLHelper.getTagValueWithoutNamespace(n)))
        {
            ...
            while (paragraph != null && (!WORD_STYLE_PARAGRAPH.equals(XMLHelper.getTagValueWithoutNamespace(paragraph)) && (!WORD_STYLE_TEXT.equals(XMLHelper.getTagValueWithoutNamespace(paragraph)))));
     ...
     * 
     * @param n , the node to jump
     * @param s , the node name to jump (w:X)
    ...
    {
        boolean change = isContainedBySpecificTag(currentNode, WORD_TAG_CHANGE_INS, WORD_STYLE_PARAGRAPH) || isContainedBySpecificTag(currentNode, WORD_TAG_CHANGE_DEL, WORD_STYLE_PARAGRAPH)
                || isContainedBySpecificTag(currentNode, WORD_TAG_CHANGE, WORD_STYLE_PARAGRAPH) || isContainedBySpecificTag(currentNode, WORD_TAG_CHANGE_FORMAT, WORD_STYLE_PARAGRAPH);
        return !change;
    }
    ...
    public static boolean isContainedBySpecificTag(Node n, String search, String limit)
    {
        ...
        while (!search.equals(XMLHelper.getTagValueWithoutNamespace(tmp)) && !limit.equals(XMLHelper.getTagValueWithoutNamespace(tmp)));
